...
using namespace std;
const string SENTINEL_STR = "$";

long long node_id;
pthread_mutex_t mutex;
pthread_cond_t cond;
queue< pair<string, string> > calc_queue;
queue< list<unsigned int> > done_queue;
...
void* thread_func(void*) {
		while (1) {
			...
			pair<string, string> cur = calc_queue.front();
			calc_queue.pop();
			pthread_mutex_unlock(&mutex);
			if (cur.first == SENTINEL_STR and cur.second == SENTINEL_STR) {
				break;
			} else {
				vector<unsigned int> res = KMPStrong(cur.first, cur.second);
				list<unsigned int> res_list;
...
int main(int argc, char** argv) {
	...
	zmq_send_msg_dontwait(info_token, node_parent_socket);

	list<unsigned int> cur_calculated;
	...
	while (awake) {
		node_token_t token;
		zmq_recieve_msg(token, node_parent_socket);
		...
		if (token.action == back) {
			if (token.id == node_id) {
				...
				if (zmq_send_recieve_wait(token_down, reply_down, node_socket) and reply_down.action == success) {
					*reply = reply_down;
				}
			}
			 ...
			 */
			zmq_init_pair_socket(node_context, node_socket);
			...
			if (zmq_send_recieve_wait(token_ping, reply_ping, node_socket) and reply_ping.action == success) {
				reply->action = success;
			}
			...
			if (token.parent_id == node_id) {
				...
				int fork_id = fork();
				if (fork_id == 0) {
					rc = execl(NODE_EXECUTABLE_NAME, NODE_EXECUTABLE_NAME, to_string(token.id).c_str(), NULL);
					assert(rc != -1);
					return 0;
				} else {
					bool ok = true;
					node_token_t reply_info({fail, token.id, token.id});
					ok = zmq_recieve_msg_wait(reply_info, node_socket);
					...
					if (has_child) {
						...
						ok = zmq_send_recieve_wait(token_bind, reply_bind, node_socket);
					...
					if (ok) {
						...
						ok = zmq_send_recieve_wait(token_ping, reply_ping, node_socket);
			...
			if (has_child) {
				if (token.id == child_id) {
					...
					ok = zmq_send_recieve_wait(token_down, reply_down, node_socket);
		...
		zmq_send_msg_dontwait(reply, node_parent_socket);
	}
	if (has_child) {
		rc = zmq_close(node_socket);
		assert(rc == 0);
